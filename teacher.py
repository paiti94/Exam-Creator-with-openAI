...
class Teacher:
    ...
    def create_full_test(self):
        ...
        num_possible_answer = int(input("How many possible answers per question would you like to have? "))
        num_questions = int(input("How many possible questions would you like to have? "))

        self.test_creator = TestGenerator(topic, num_possible_answer, num_questions)
    ...
    def create_student_view(selfself, test, num_questions):
        student_views = {1:""}
        question_number = 1
        for line in test.split("\n"):
            if not line.startswith("Correct Answer:"):
                student_views[question_number] += line+"\n"
            else:
                if question_number < num_questions:
                    question_number+=1
                    student_views[question_number] = ""
